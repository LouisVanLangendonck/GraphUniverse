"""
GPS (Graph PolyMer with Sampling) Encoder Implementation
...
if __name__ == "__main__":
    # Create a GPS encoder with GIN local convolution
    encoder_gin = GPSEncoder(
        input_dim=16,
        hidden_dim=64,
        num_layers=4,
        pe_dim=0,  # No PE
    ...
    # Create a GPS encoder with PNA local convolution
    encoder_pna = GPSEncoder(
        input_dim=16,
        hidden_dim=64,
        num_layers=4,
        pe_dim=0,  # No PE
    ...
    print("GPS encoders created successfully!")
    print(f"GIN encoder parameters: {sum(p.numel() for p in encoder_gin.parameters()):,}")
    print(f"PNA encoder parameters: {sum(p.numel() for p in encoder_pna.parameters()):,}")