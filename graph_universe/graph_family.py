...
class GraphFamilyGenerator:
    ...
    def generate_family(
        ...
        unique_set_of_community_combinations = {
            tuple(sorted(arr)) for arr in self.community_labels_per_graph
        }

        # Convert back to list of lists
        unique_list_of_community_combinations = [
            list(tup) for tup in unique_set_of_community_combinations
        ]
    ...
    def _validate_parameters(self) -> None:
        ...
        if (
            len(self.degree_separation_range) != 2
            or self.degree_separation_range[0] > self.degree_separation_range[1]
        ):
    ...
    def _sample_graph_parameters(self) -> dict[str, Any]:
        ...
        target_average_degree = np.random.uniform(
        ...
        while attempts < max_attempts:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Attempt {attempts}: GraphSample initialization failed: {e!s}")
                    raise e
    ...
    def _collect_generation_stats(
        self, start_time: float, failed_graphs: int, n_graphs: int
    ) -> None:
        ...
        if successful_graphs > 0:
            ...
            successful_metadata = [
                m for m in self.generation_metadata if not m.get("failed", False)
            ]
    ...
    def analyze_graph_family_properties(self) -> dict[str, Any]:
        ...
        if self.graphs is None:
            raise ValueError(
                "No graphs in family. Please generate family first before analyzing properties."
            )
        ...
        for graph in tqdm(
            self.graphs, desc="Analyzing graph family properties", total=len(self.graphs)
        ):
            ...
            try:
                ...
                warnings.warn(f"Failed to calculate clustering coefficient: {e}", stacklevel=2)
            ...
            if graph.n_nodes > 0:
                deviations = graph._calculate_community_deviations(
                    graph.graph, graph.community_labels, graph.P_sub
                )
    ...
    def analyze_graph_family_signals(self) -> dict[str, Any]:
        ...
        if not self.graphs:
            raise ValueError(
                "No graphs in family. Please generate family first before analyzing signals."
            )
    ...
    def analyze_graph_family_consistency(self) -> dict[str, Any]:
        ...
        if not self.graphs:
            raise ValueError(
                "No graphs in family. Please generate family first before analyzing consistency."
            )
    ...
    def _calculate_generation_fidelity(self) -> list[float]:
        ...
        for graph in self.graphs:
            try:
                ...
                if (
                    len(expected_flat) > 1
                    and np.std(expected_flat) > 0
                    and np.std(actual_flat) > 0
                    and not np.isnan(pearsonr(expected_flat, actual_flat)[0])
                ):
    ...
    def _calculate_feature_consistency(self) -> list[float]:
        ...
        for graph_idx, graph in enumerate(self.graphs):
            ...
            for universe_comm_id in graph.communities:
                ...
                current_nodes = np.where(graph.community_labels_universe_level == universe_comm_id)[
                    0
                ]
                ...
                for other_graph_idx, other_graph in enumerate(self.graphs):
                    ...
                    if universe_comm_id in other_graph.communities:
                        other_nodes = np.where(
                            other_graph.community_labels_universe_level == universe_comm_id
                        )[0]
    ...
    def _get_expected_ordering(
        self, communities: list[int], degree_centers: np.ndarray
    ) -> np.ndarray:
    ...
    def _fit_power_law_exponent(self, degrees: list[int]) -> float:
        ...
        try:
            ...
            warnings.warn(f"Failed to fit power law exponent: {e}", stacklevel=2)
            return 0.0
