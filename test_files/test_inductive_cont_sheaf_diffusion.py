#!/usr/bin/env python3
"""
Inductive test script for continuous neural sheaf diffusion models.
...
class SimpleGCN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=2, dropout=0.1):
        ...
        # # Add Sigmoid for classification tasks
...
def train_inductive_cont_sheaf_model(model, data, device, epochs=30, lr=0.01, weight_decay=5e-4):
    """Train the inductive continuous sheaf diffusion model."""
    print(f"Training inductive continuous sheaf diffusion model for {epochs} epochs...")
    ...
    if param_count == 0:
        raise ValueError("Continuous sheaf model has no parameters")
...
def main():
    """Main function to run the inductive continuous test."""
    print("=" * 60)
    print("INDUCTIVE CONTINUOUS NEURAL SHEAF DIFFUSION COMMUNITY PREDICTION")
    print("=" * 60)
    ...
    try:
        ...
        sheaf_configs = [
            {'sheaf_type': 'diag', 'd': 2, 'name': 'Continuous Diagonal Sheaf (d=2)', 'orth': 'cayley'},
            {'sheaf_type': 'bundle', 'd': 2, 'name': 'Continuous Bundle Sheaf (d=2)', 'orth': 'cayley'},
            {'sheaf_type': 'general', 'd': 2, 'name': 'Continuous General Sheaf (d=2)', 'orth': 'cayley'},
        ]
        ...
        for config in sheaf_configs:
            ...
            try:
                ...
                # Train model
                start_time = time.time()
                result = train_inductive_cont_sheaf_model(model, data, device, epochs=30)
        ...
        print(f"\nInductive continuous test completed successfully!")
