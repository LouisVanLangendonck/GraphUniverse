#!/usr/bin/env python3
"""
Test script for continuous neural sheaf diffusion models and Bundle Neural Networks.
...
from experiments.neural_sheaf_diffusion.cont_models import (
...
# Import BuNN from the test file
from test_bunn import BuNN, create_bunn_for_node_classification
...
def create_cont_sheaf_model(model_type, edge_index, args):
    """Create a continuous sheaf diffusion model."""
    # Ensure edge_index is on the correct device
    device = args['device']
    edge_index = edge_index.to(device)
    
    if model_type == 'graph_laplacian':
        return GraphLaplacianDiffusion(edge_index, args)
    elif model_type == 'diag_sheaf':
        return DiagSheafDiffusion(edge_index, args)
    elif model_type == 'bundle_sheaf':
        return BundleSheafDiffusion(edge_index, args)
    elif model_type == 'general_sheaf':
        return GeneralSheafDiffusion(edge_index, args)
    else:
        raise ValueError(f"Unknown model type: {model_type}")


def train_cont_sheaf_model(model, data, device, epochs=20, lr=0.01, weight_decay=5e-4):
    """Train the continuous sheaf diffusion model."""
    print(f"Training continuous sheaf diffusion model for {epochs} epochs...")
    ...
    print(f"Continuous sheaf model initialized with {param_count} parameters")
    
    if param_count == 0:
        raise ValueError("Continuous sheaf model has no parameters after initialization")
...
def create_bunn_model(config, input_dim, output_dim, hidden_dim=64):
    """Create a Bundle Neural Network model."""
    return create_bunn_for_node_classification(
        input_dim=input_dim,
        num_classes=output_dim,
        hidden_dim=hidden_dim,
        num_layers=config.get('num_layers', 2),
        d=config.get('d', 2),
        num_bundles=config.get('num_bundles', 8),
        bundle_method=config.get('bundle_method', 'rotation'),
        heat_method=config.get('heat_method', 'taylor'),
        max_degree=config.get('max_degree', 10),
        diffusion_times=config.get('diffusion_times', [0.5, 1.0]),
        dropout=config.get('dropout', 0.1),
        activation=config.get('activation', 'relu')
    )


def train_bunn_model(model, data, device, epochs=100, lr=0.01, weight_decay=5e-4):
    """Train the Bundle Neural Network model."""
    print(f"Training Bundle Neural Network for {epochs} epochs...")
    ...
    if param_count == 0:
        raise ValueError("BuNN model has no parameters after initialization")
    ...
    model.eval()
    with torch.no_grad():
        initial_out = model(x, edge_index)
        initial_pred = initial_out.argmax(dim=1)
        print(f"\nInitial predictions distribution: {torch.bincount(initial_pred)}")
        print(f"True labels distribution: {torch.bincount(y)}")
        print(f"Initial output range: [{initial_out.min():.4f}, {initial_out.max():.4f}]")
...
def main():
    ...
    print("=" * 60)
    print("CONTINUOUS NEURAL SHEAF DIFFUSION & BUNDLE NEURAL NETWORK COMMUNITY PREDICTION")
    print("=" * 60)
    ...
    try:
        ...
        # Test different continuous sheaf types
        sheaf_configs = [
            {'model_type': 'graph_laplacian', 'd': 1, 'name': 'Graph Laplacian Diffusion (d=1)'},
            {'model_type': 'diag_sheaf', 'd': 2, 'name': 'Diagonal Sheaf Diffusion (d=2)'},
            {'model_type': 'diag_sheaf', 'd': 4, 'name': 'Diagonal Sheaf Diffusion (d=4)'},
            {'model_type': 'bundle_sheaf', 'd': 2, 'name': 'Bundle Sheaf Diffusion (d=2)'},
            {'model_type': 'general_sheaf', 'd': 2, 'name': 'General Sheaf Diffusion (d=2)'},
        ]
        
        # Test different BuNN configurations
        bunn_configs = [
            {
                'name': 'BuNN (d=2, bundles=16, rotation)',
                'd': 2,
                'num_bundles': 16,
                'bundle_method': 'rotation',
                'heat_method': 'taylor',
                'num_layers': 2,
                'diffusion_times': [0.5, 1.0],
                'dropout': 0.1,
                'activation': 'elu'
            },
            {
                'name': 'BuNN (d=2, bundles=32, rotation)',
                'd': 2,
                'num_bundles': 32,
                'bundle_method': 'rotation',
                'heat_method': 'taylor',
            ...
            {
                'name': 'BuNN (d=2, bundles=8, householder)',
                'd': 2,
                'num_bundles': 8,
                'bundle_method': 'householder',
                'heat_method': 'taylor',
                'num_layers': 2,
                'diffusion_times': [0.5, 1.0],
                'dropout': 0.1,
            ...
            {
                'name': 'BuNN (d=2, bundles=8, spectral)',
                'd': 2,
                'num_bundles': 8,
                'bundle_method': 'rotation',
                'heat_method': 'spectral',
                'num_layers': 2,
                'diffusion_times': [0.5, 1.0],
                'dropout': 0.1,
                'activation': 'elu'
        ...
        # Test continuous sheaf models
        print(f"\n" + "="*60)
        print("TESTING CONTINUOUS SHEAF DIFFUSION MODELS")
        print("="*60)
        ...
        for config in sheaf_configs:
            ...
            try:
                ...
                result = train_cont_sheaf_model(model, data, device, epochs=15)
        ...
        # Test BuNN models
        print(f"\n" + "="*60)
        print("TESTING BUNDLE NEURAL NETWORKS")
        print("="*60)
        ...
        for config in bunn_configs:
            ...
            try:
                # Create model
                model = create_bunn_model(config, input_dim, output_dim, hidden_dim)
                
                # Train model
                start_time = time.time()
                result = train_bunn_model(model, data, device, epochs=15)
                training_time = time.time() - start_time
