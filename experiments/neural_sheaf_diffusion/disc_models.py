...
from experiments.neural_sheaf_diffusion.laplacian_builders import DiagLaplacianBuilder, NormConnectionLaplacianBuilder, GeneralLaplacianBuilder
from experiments.neural_sheaf_diffusion.sheaf_models import LocalConcatSheafLearner, EdgeWeightLearner, LocalConcatSheafLearnerVariant


class DiscreteDiagSheafDiffusion(SheafDiffusion):

    def __init__(self, edge_index, config):
        super(DiscreteDiagSheafDiffusion, self).__init__(edge_index, config)
        assert config['d'] > 0
        ...
        self.laplacian_builder = DiagLaplacianBuilder(self.graph_size, edge_index, d=self.d,
    ...
    def create_with_new_edge_index(self, edge_index: torch.Tensor):
        # Create a new instance with updated edge_index, but reuse config and parameters
        config = copy.deepcopy(self.config)
        config["graph_size"] = int(edge_index.max().item()) + 1

        new_model = self.__class__(edge_index=edge_index, config=config)
        new_model.load_state_dict(self.state_dict())  # copy weights
        return new_model

    def forward(self, x):
        # FIXED: Get actual number of nodes dynamically
        ...
        # FIXED: Use actual number of nodes instead of self.graph_size
        ...
        for layer in range(self.layers):
            if layer == 0 or self.nonlinear:
                ...
                # FIXED: Reshape using actual number of nodes
                maps = self.sheaf_learners[layer](x_maps.reshape(actual_num_nodes, -1), self.edge_index)
            ...
            # FIXED: Use actual number of nodes for epsilon tiling
...
class DiscreteBundleSheafDiffusion(SheafDiffusion):
    ...
    def __init__(self, edge_index, config):
        super(DiscreteBundleSheafDiffusion, self).__init__(edge_index, config)
        assert config['d'] > 1
        ...
        self.laplacian_builder = NormConnectionLaplacianBuilder(
    ...
    def forward(self, x):
        ...
        # FIXED: Use actual number of nodes
        x = x.view(actual_num_nodes * self.final_d, -1)
        ...
        for layer in range(self.layers):
            ...
            # FIXED: Pass actual_num_nodes to left_right_linear
...
class DiscreteGeneralSheafDiffusion(SheafDiffusion):
    ...
    def __init__(self, edge_index, config):
        super(DiscreteGeneralSheafDiffusion, self).__init__(edge_index, config)
        assert config['d'] > 1
