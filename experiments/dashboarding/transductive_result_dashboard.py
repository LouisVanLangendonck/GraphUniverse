...
st.set_page_config(page_title="GraphWorld Transductive Benchmarking Visualization", layout="wide")

@st.cache_data
def load_experiment_data(results_dir):
    """Load and parse all experiment results from a directory recursively."""
    all_results = []
    summary_stats = {
    ...
    for root, dirs, files in os.walk(results_dir):
        ...
        if 'result.json' in files and 'config.json' in files:
            config_path = os.path.join(root, 'config.json')
            result_path = os.path.join(root, 'result.json')
            
            summary_stats['total_runs_attempted'] += 1
            ...
            try:
                ...
                # Add config and result data together
                combined_data = {
                    'run_id': os.path.basename(root),
                    'config_path': config_path,
                    **config,  # Add all config parameters
                    **result   # Add all result data
                }
                
                all_results.append(combined_data)
                summary_stats['successful_runs'] += 1
                summary_stats['total_time'] += result.get('run_time', 0)
                
            except Exception as e:
                summary_stats['failed_runs'] += 1
                print(f"Error loading {root}: {str(e)}")
    
    # Calculate success rate
    if summary_stats['total_runs_attempted'] > 0:
        summary_stats['success_rate'] = summary_stats['successful_runs'] / summary_stats['total_runs_attempted']
        summary_stats['avg_time_per_run'] = summary_stats['total_time'] / summary_stats['successful_runs']
    else:
        summary_stats['success_rate'] = 0
        summary_stats['avg_time_per_run'] = 0
    ...
    for result in data['all_results']:
        try:
            base_row = {
                ...
                'degree_distribution': result.get('degree_distribution', 'unknown'),
                'n_graphs': result.get('n_graphs', 0),
                'total_time': result.get('total_time', 0),
                'universe_K': result.get('universe_K', 0)
            }
            
            # Add all config parameters as columns
            config_params = [
                'num_nodes', 'num_communities', 'universe_K', 'universe_feature_dim',
                'universe_edge_density', 'universe_homophily', 'universe_randomness_factor',
                'cluster_count_factor', 'center_variance', 'cluster_variance',
                'assignment_skewness', 'community_exclusivity', 'degree_heterogeneity',
                'edge_noise', 'power_law_x_min', 'learning_rate', 'weight_decay',
                'epochs', 'patience', 'batch_size', 'hidden_dim', 'num_layers',
                'dropout', 'eps', 'transformer_num_heads', 'transformer_max_nodes',
                'transformer_max_path_length'
            ]
            
            for param in config_params:
                if param in result:
                    base_row[param] = result[param]
            
            # Add sweep parameters
            for key, value in result.get('sweep_parameters', {}).items():
                base_row[f'sweep_{key}'] = value
                
            # Add random parameters
            for key, value in result.get('random_parameters', {}).items():
                base_row[f'random_{key}'] = value
                
            # Add family properties
            for key, value in result.get('family_properties', {}).items():
                if isinstance(value, (int, float)):
                    base_row[f'family_{key}'] = value
                    
            # Add family consistency scores
            for key, value in result.get('family_consistency', {}).items():
                if isinstance(value, dict) and 'score' in value:
                    base_row[f'consistency_{key}'] = value['score']
                    
            # Add community signals
            for signal_type, signal_data in result.get('community_signals', {}).items():
            ...
            # Add model results and optimal hyperparameters
            if 'results' in result:
                for task_name, task_results in result['results'].items():
                    for model_name, model_data in task_results.items():
                        if model_data.get('success', True):  # Default to True if not specified
                            # Add test metrics
                            for metric, value in model_data.get('test_metrics', {}).items():
                                if value is not None:  # Only add non-None values
                                    base_row[f'{task_name}-{model_name}-{metric}'] = value
                            
                            # Add training time
                            train_time = model_data.get('train_time', 0)
                            if train_time is not None:  # Only add non-None values
                                base_row[f'{task_name}-{model_name}-train_time'] = train_time
                            
                            # Add optimal hyperparameters
                            if 'optimal_hyperparams' in model_data:
                                for param_name, param_value in model_data['optimal_hyperparams'].items():
                                    if param_value is not None:  # Only add non-None values
                                        base_row[f'{task_name}-{model_name}-opt_{param_name}'] = param_value
...
def get_available_tasks_and_metrics(df):
    ...
    for col in df.columns:
        if '-' in col and not col.startswith(('sweep_', 'random_', 'family_', 'signal_')):
            parts = col.split('-')
            if len(parts) >= 3:
...
def main():
    st.title("🧠 GraphUniverse Transductive Benchmarking Visualization")
    
    # Sidebar for directory selection
    st.sidebar.header("📁 Data Selection")
    
    # Add directory input
    default_dir = "multi_transductive_results"
    ...
    if results_path.exists():
        ...
        if len(df) > 0:
            ...
            st.sidebar.info(f"📊 Total runs attempted: {data.get('summary_stats', {}).get('total_runs_attempted', 0)}")
            st.sidebar.info(f"⏱️ Success rate: {data.get('summary_stats', {}).get('success_rate', 0):.1%}")
            ...
            with tab1:
                ...
                # Get available parameters - now including config parameters and optimal hyperparameters
                param_columns = [col for col in df.columns if (
                    col.startswith(('sweep_', 'random_', 'family_', 'consistency_', 'signal_')) or 
                    col in ['num_nodes', 'num_communities', 'universe_K', 'universe_feature_dim',
                           'universe_edge_density', 'universe_homophily', 'universe_randomness_factor',
                           'cluster_count_factor', 'center_variance', 'cluster_variance',
                           'assignment_skewness', 'community_exclusivity', 'degree_heterogeneity',
                           'edge_noise', 'power_law_x_min', 'learning_rate', 'weight_decay',
                           'epochs', 'patience', 'batch_size', 'hidden_dim', 'num_layers',
                           'dropout', 'eps', 'transformer_num_heads', 'transformer_max_nodes',
                           'transformer_max_path_length'] or
                    col.startswith(('community-', 'k_hop_community_counts-')) and '-opt_' in col
                )]
                ...
                with col1:
                    ...
                    if x_param is None:
                        st.error("Please select an X-axis parameter")
                        return
                ...
                if selected_models:
                    ...
                    for model in selected_models:
                        ...
                        if model_col in df.columns:
                            ...
                            st.warning(f"Column not found: {model_col}")
                    
                    if not relevant_columns:
                        st.error(f"No data found for the selected models and metric: {selected_task}-{y_metric}")
                        return
                    
                    # Debug: Show available columns
                    st.sidebar.write("Debug - Available columns:", df.columns.tolist())
                    st.sidebar.write("Debug - Selected columns:", relevant_columns)
                    st.sidebar.write("Debug - X parameter:", x_param)
                    
                    # Create a mask for rows that have all required data
                    mask = df[relevant_columns].notna().all(axis=1)
                    df_plot = df[mask].copy()
                    
                    # Debug: Show data info
                    st.sidebar.write("Debug - Original data shape:", df.shape)
                    st.sidebar.write("Debug - Filtered data shape:", df_plot.shape)
                    st.sidebar.write("Debug - X parameter in columns:", x_param in df_plot.columns)
                    ...
                    if len(df_plot) < len(df):
                        st.info(f"ℹ️ Using {len(df_plot)} runs with complete data (filtered out {len(df) - len(df_plot)} runs with missing values)")
                    
                    try:
                        # Debug: Show data for plotting
                        st.sidebar.write("Debug - Data for plotting:")
                        st.sidebar.write(df_plot[[x_param] + relevant_columns].head())
                        ...
                        for model in selected_models:
                            ...
                            if model_col in df_plot.columns:
                                # Debug: Show data for this model
                                st.sidebar.write(f"Debug - Data for {model}:")
                                st.sidebar.write(df_plot[[x_param, model_col]].head())
                                ...
                                st.warning(f"Column not found in filtered data: {model_col}")
                        ...
                        st.error(f"DataFrame info:\n{df_plot.info()}")
                        st.error(f"DataFrame head:\n{df_plot.head()}")
                        st.error(f"Selected columns: {relevant_columns}")
                        st.error(f"X parameter: {x_param}")
                        st.error("Please check if the selected parameters and metrics exist in the data.")
        else:
    ...
    with tab4:
        ...
        with col1:
            ...
            st.json({
                "Total Runs": data.get('summary_stats', {}).get('total_runs_attempted', 0),
                "Successful Runs": data.get('summary_stats', {}).get('successful_runs', 0),
                "Failed Runs": data.get('summary_stats', {}).get('failed_runs', 0),
                "Success Rate": f"{data.get('summary_stats', {}).get('success_rate', 0):.1%}",
                "Total Time": f"{data.get('summary_stats', {}).get('total_time', 0):.1f}s",
                "Avg Time per Run": f"{data.get('summary_stats', {}).get('avg_time_per_run', 0):.1f}s"
            })
        ...
        with col2:
            ...
            if param_columns:
                ...
                st.info("No parameter columns found in the data")
    ...
    with tab5:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"{results_dir}_filtered_results.csv",
            mime="text/csv"
        )
    ...
    with tab6:
        ...
        for result in data['all_results']:
            if 'results' in result:
                for task_name, task_results in result['results'].items():
                    for model_name, model_data in task_results.items():
                        # Check if model has optimal hyperparameters
        ...
        if not models_with_hyperopt:
            ...
            # Debug information
            st.sidebar.write("Debug - Available tasks and models:")
            for result in data['all_results']:
                if 'results' in result:
                    for task_name, task_results in result['results'].items():
                        for model_name, model_data in task_results.items():
                            st.sidebar.write(f"Task: {task_name}, Model: {model_name}")
                            st.sidebar.write(f"Has optimal_hyperparams: {'optimal_hyperparams' in model_data}")
                            if 'optimal_hyperparams' in model_data:
                                st.sidebar.write(f"Hyperparams: {model_data['optimal_hyperparams']}")
        else:
            # Model selection
            col1, col2 = st.columns(2)
            with col1:
            ...
            hyperparams = set()
            for result in data['all_results']:
                if 'results' in result and selected_task in result['results'] and selected_model in result['results'][selected_task]:
                    model_data = result['results'][selected_task][selected_model]
                    if model_data.get('success') and 'optimal_hyperparams' in model_data:
                    ...
                    if 'results' in result and selected_task in result['results'] and selected_model in result['results'][selected_task]:
                        ...
                        if model_data.get('success') and 'optimal_hyperparams' in model_data:
                            if selected_hyperparam in model_data['optimal_hyperparams']:
                                ...
                                sweep_params.append({
                                    'universe_homophily': result.get('universe_homophily'),
                                    'universe_edge_density': result.get('universe_edge_density')
                                })
